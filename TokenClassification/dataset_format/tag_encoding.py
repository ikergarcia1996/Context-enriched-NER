...
def remove_misc_label(tags: List[str]) -> List[str]:
    new_tags: List[str] = []
    for i in range(len(tags)):
        tag = tags[i]
        if tag == "O":
            ...
            try:
                ...
                raise ValueError(
                    f"Error tag {tag}, unable to split the tag in 2 fields. Tags: {tags}"
                )
...
def rewrite_tags(tags, encoding: str = "iob2", remove_misc: bool = False) -> List[str]:
    if encoding.lower() == "iob":
        new_tags = to_iob_encoding(tags)
    elif encoding.lower() == "iob2":
        new_tags = to_iob2_encoding(tags)
    elif encoding.lower() == "bilou":
        new_tags = to_bilou_encoding(tags)
    else:
    ...
    if remove_misc:
        new_tags = remove_misc_label(new_tags)
